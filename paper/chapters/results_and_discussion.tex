...
The phoneme correspondence for vowels is sometimes ambiguous and
nondeterministic, splitting a vowel pronunciation into either its long or short
version. Some examples: ``cake'' (``keyk'') and ``cat'' (``kat''),
``me''(``mi'') and ``bed''(``bed''), ``time''(``taym'') and ``sit''(``sit''),
``home''(``hom'') and ``hot''(``hat''), ``cube''(``kyub'') and
``cup''(``kap''). This ambiguity results in a problem where a vowel must be
mapped to multiple different pronunciations.

Possible limitations may include the lack of professional linguistic input
towards the creation of datasets and orthographic nativization rules to help
ensure consistency and accuracy of the system output.