...
their own versions of each word and selected the one that all annotators agreed fit
best.

Metrics were measured through Word Error Rate (WER) and Token Error Rate (TER). WER measured
the error per whole word (comparing the actual output to our expected output in the dataset),
 while TER measured the error per token (character).