...
CER uses the \textbf{Levenshtein distance}, also known as edit distance, to calculate how many
edits it would take to transform the actual output to the correct output. For example, changing
``cot'' to ``dog'' would take two edits (\textbf{c $\rightarrow$ d} then \textbf{t $\rightarrow$ g}).

By using CER alongside WER, we can measure both word-level and character-level performance. WER
helps us on a broader level while CER helps with the smaller details that can really change
