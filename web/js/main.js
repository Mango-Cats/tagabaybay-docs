...
function renderAdviserAndCollaborators() {
    ...
    if (collabEl && collabList) {
      ...
      <li>${escapeHtml(typeof name === "string" ? name : name.name)}</li>
    `).join("");
    }
}
...
function initGlobalDelegation() {
    document.addEventListener("click", (e) => {
        ...
        if (!target)
            return;
        const bioBtn = target.closest(".btn-bio");
        if (bioBtn) {
            e.preventDefault();
        ...
        const copyBtn = target.closest(".btn-copy-email");
        if (copyBtn) {
            e.preventDefault();
...
function openBioModal(memberId) {
    ...
    if (avatarEl)
        avatarEl.innerHTML = renderAvatar(member.avatar, member.name);
    if (nameEl)
        nameEl.textContent = member.name;
    if (deptEl)
        deptEl.textContent = member.affiliation;
...
function initDemo() {
    ...
    const wordButtons = document.querySelectorAll(".word-choice-btn");
    if (!wordButtons || wordButtons.length === 0)
        return;
    function selectWord(word) {
        ...
        if (!data)
            return;
        if (outputEl)
            outputEl.textContent = data.nativized;
        if (ipaEl)
